...
# define VLD_OPARRAY_FILENAME(opa) ZSTR_VAL((opa)->filename)
# define VLD_OPARRAY_FUNCTION_NAME(opa) ((opa)->function_name ? ZSTR_VAL((opa)->function_name) : NULL)
# define VLD_EXEC_OP_ARRAY(ex) (&(ex)->func->op_array)
#else
# define VLD_OPARRAY_FILENAME(opa) ((opa)->filename)
# define VLD_OPARRAY_FUNCTION_NAME(opa) ((opa)->function_name)
# define VLD_EXEC_OP_ARRAY(ex) ((ex)->op_array)
#endif
...
static zend_op_array *vld_compile_file(zend_file_handle *file_handle, int type TSRMLS_DC)
{
	...
	if (VLD_G(path_dump_file)) {
		fprintf(VLD_G(path_dump_file), "subgraph cluster_file_%p { label=\"file %s\";\n", op_array, VLD_OPARRAY_FILENAME(op_array) ? VLD_OPARRAY_FILENAME(op_array) : "__main");
	}
	if (op_array) {
...
static void fix_op_array(zend_op_array *op_array TSRMLS_DC)
{
	...
	function_storage.type = ZEND_USER_FUNCTION;
	function_storage.op_array = *op_array;
	execute_data_storage.func = &function_storage;
	execute_data = &execute_data_storage;
#else
	allocated_execute_data = (zend_execute_data *)zend_vm_stack_alloc(
		ZEND_MM_ALIGNED_SIZE(sizeof(zend_execute_data)) +
		ZEND_MM_ALIGNED_SIZE(sizeof(zval**) * op_array->last_var * (EG(active_symbol_table) ? 1 : 2)) +
		ZEND_MM_ALIGNED_SIZE(sizeof(temp_variable)) * op_array->T TSRMLS_CC);
	allocated_execute_data->op_array = op_array;
	...
	for (i = 0; i < op_array->last; i++)
	{
		...
		if ((void *) zend_vm_get_opcode_handler(op_array->opcodes[i].opcode, &(op_array->opcodes[i])) ==  (void *)execute_data->opline->handler )
		{
			continue;
		}
...
#endif
{
	...
	if (VLD_G(sg_decode) && op_array)
	{
		...
		if (strncmp(executed_filename, VLD_OPARRAY_FILENAME(op_array), strlen(executed_filename)) == 0) {

			// don't dump source guardian encoded stuff
			if (execute_count == 1)
			{
				// first, fix opcodes not contained in a function or class
				if (VLD_OPARRAY_FUNCTION_NAME(op_array) == NULL || strlen(VLD_OPARRAY_FUNCTION_NAME(op_array)) == 0) {
